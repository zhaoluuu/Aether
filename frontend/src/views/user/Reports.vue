        ...
        <div v-if="activeTab === 'detail'">
          <Card class="overflow-hidden">
            ...
            <Pagination
              ...
              :page-size-options="DAILY_PAGE_SIZE_OPTIONS"
              cache-key="reports-detail-page-size"
        ...
        <div v-else-if="activeTab === 'models'">
          <Card class="overflow-hidden">
            ...
            <Pagination
              ...
              :page-size-options="DAILY_PAGE_SIZE_OPTIONS"
              cache-key="reports-model-page-size"
...
import { Card, Badge, Skeleton, Tabs, TabsList, TabsTrigger, Table, TableHeader, TableBody, TableRow, TableHead, TableCell, Pagination, Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from '@/components/ui'
import { TimeRangePicker } from '@/components/common'
import { useReportsData } from '@/composables/useReportsData'
import type { ModelSummary } from '@/composables/useReportsData'
...
const { timeRange, apiKeyFilter, apiKeyOptions, summary, bucketStats, bucketBreakdowns, modelSummary, loading, loadError, hasLoaded, resolvedGranularity } = useReportsData()
...
const DAILY_PAGE_SIZE_OPTIONS = [10, 20, 50]
...
const selectedApiKeyLabel = computed(() => {
  if (selectedApiKeyValue.value === '__all__') return ''
  return apiKeyOptions.value.find(option => option.value === selectedApiKeyValue.value)?.label || ''
})
...
function setDetailGranularity(granularity: GranularityOption) {
...
function buildExportFilename(baseName: string, includeGranularity = true): string {
  const keyPrefix = selectedApiKeyLabel.value
    ? `${sanitizeFilenameSegment(selectedApiKeyLabel.value)}-`
    : ''
  const rangeLabel = getExportRangeLabel()
  if (!includeGranularity) {
    return `${keyPrefix}${baseName}-${rangeLabel}`
  }
  return `${keyPrefix}${baseName}-${rangeLabel}-${getExportGranularityLabel()}`
}
...
function formatBucketLabel(bucketStart: string, bucketEnd: string) {
...
function formatBucketExportLabel(bucketStart: string, bucketEnd: string) {
  const bucketDate = bucketStart.slice(0, 10)
...
function buildCompositionLines(summary: Pick<DailyUsageBreakdown, 'inputTokens' | 'outputTokens' | 'cacheCreationTokens' | 'cacheReadTokens'>): [string, string] {
  return [
    `输入 ${formatTokens(summary.inputTokens)} · 输出 ${formatTokens(summary.outputTokens)}`,
    `缓存创建 ${formatTokens(summary.cacheCreationTokens)} · 缓存读取 ${formatTokens(summary.cacheReadTokens)}`,
  ]
}

function getBucketCompositionLines(bucketStart: string): [string, string] {
  const summary: Pick<DailyUsageBreakdown, 'inputTokens' | 'outputTokens' | 'cacheCreationTokens' | 'cacheReadTokens'> = bucketBreakdowns.value[bucketStart] ?? {
    inputTokens: 0,
    outputTokens: 0,
  ...
  return buildCompositionLines(summary)
}
